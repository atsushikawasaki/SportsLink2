...
- Vite 5.0.8（ビルドツール）
- PWA（Service Worker + IndexedDB via Dexie 3.2.4）
- Tailwind CSS 3.3.6
- Zustand 4.4.7（状態管理）
- React Router DOM 6.30.1
- React Hook Form 7.48.2 + Zod 3.22.4（フォーム管理）
- Supabase Client SDK 2.38.4
...
- Supabase JS SDK 2.38.4
- Passport JWT 4.0.1（認証）
- Bcrypt 5.1.1（パスワードハッシュ）
- Swagger 7.1.16（API ドキュメント）
...
- POST /auth/reset-password - パスワードリセット（トークン検証後）
...
- POST /support/contact - 問い合わせ送信
- GET /support/faq - FAQ一覧取得（将来実装）
- GET /support/help - ヘルプ記事取得（将来実装）
...
- 機能:
  ...
  - ログインボタン
  - サインアップ画面へのリンク
  - **「パスワードを忘れた場合」リンク**（`/support/forgot-password`へ遷移）
  - エラーメッセージ表示
  - ログイン成功時: JWTとユーザー情報をlocalStorageに保存、ダッシュボードへ遷移
...
##### ナビゲーション
- **固定ナビゲーション**: ダッシュボード、大会一覧、チーム一覧、分析（Coming Soon、未実装）、設定
- **戻るボタン**: 詳細画面・管理画面に表示、適切な戻り先へ遷移
- **通知ボタン**: ヘッダーに表示（未実装、console.logのみ）
- **パンくずリスト**: 全画面に実装（現在のページ階層を表示、各階層へのリンク）
  - 例: ダッシュボード > 大会一覧 > 大会詳細 > エントリー管理
  - クリック可能なリンクで各階層に遷移可能
- **ユーザーメニュー**: プロフィール表示、ログアウト
...
- パス: `/scoring`（試合一覧）、`/scoring/:matchId`（特定試合）
- 機能:
  - **認証キー入力**（敗者審判モード時）:
...
- 機能:
  - **フォーム項目**:
    - チーム選択（ドロップダウン、全チーム一覧）
    - 選手名（必須）
    - ポジション（前衛/後衛、必須）
  - **保存**: 選手追加API呼び出し、成功時は`/teams/:teamId/players`へ遷移（未実装のため、実際はチーム一覧へ遷移）
  - **キャンセル**: チーム一覧へ戻る（navigate(-1)）
...
#### クリティカルな不足機能（アカウント周りの不備）
1. **パスワードリセット機能**:
   - ログイン画面に「パスワードを忘れた場合」のリンクが未実装
   - パスワードリセットフロー:
     - メールアドレス入力画面
     - リセットメール送信
     - リセットトークン検証
...
2. **利用規約・プライバシーポリシーの同意フロー**:
   - サインアップ時に利用規約・プライバシーポリシーへの同意チェックボックスが未実装
   - 規約・ポリシーの表示画面（モーダルまたは別ページ）
   - 同意ログの記録（同意日時、規約バージョン）
   - 規約更新時の再同意フロー

3. **アカウント削除（退会）機能**:
   - 設定画面にアカウント削除ボタンが未実装
   - 退会確認ダイアログ（「すべてのデータが削除されます」）
   - 退会処理:
     - ユーザーデータの削除または匿名化
     - 関連データの処理（大会、試合、スコア等）
...
#### 未実装機能
1. **通知機能**:
   - Navigation.tsxに通知ボタンがあるが、通知画面・機能が未実装
   - 通知一覧表示、通知設定の実装が必要
...
3. **選手一覧画面**:
   - PlayerRegistrationで`/teams/:teamId/players`への遷移があるが、このルートが未定義
   - チームごとの選手一覧画面の実装が必要

4. **スコア入力一覧画面**:
   - `/scoring`ルートは定義されているが、matchId必須のため実質未使用
   - 試合一覧からのスコア入力選択画面の実装が必要

#### 改善が必要な機能（TODOコメントあり）
1. **チーム名取得**:
   - 複数のコンポーネントでチーム名が「チーム1」「チーム2」とハードコード
   - 影響箇所:
     - Dashboard.tsx（リアルタイム試合表示）
     - AssignedMatches.tsx（担当試合一覧）
     - MatchAssignment.tsx（試合割当）
     - LiveScoreView.tsx（リアルタイム観戦）
   - 改善: Match_Pairsから実際のチーム名を取得する実装が必要
...
#### 機能追加の推奨事項
1. **大会公開・終了ボタン**:
   - 大会詳細画面に公開・終了ボタンを追加（現在は編集画面から変更）
   - ワンクリックで公開・終了できるように改善
