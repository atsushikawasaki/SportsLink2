...
describe('changeUmpire', () => {
  ...
  it('should return 400 when umpire_id is missing', async () => {
    const request = new Request('http://localhost/api/matches/match-123/umpire', {
      method: 'PUT',
      body: JSON.stringify({}),
    });

    const response = await changeUmpire('match-123', request);
    const data = await response.json();

    expect(response.status).toBe(400);
    expect(data.error).toContain('審判IDは必須です');
  ...
  it('should return 403 when user has no permission', async () => {
    ...
    expect(data.error).toContain('この試合の審判を変更する権限がありません');
  ...
  it('should change umpire and return 200', async () => {
    mockSingle.mockResolvedValueOnce({ data: { tournament_id: 'tournament-123' }, error: null });
    mockSingle.mockResolvedValueOnce({
      data: { id: 'match-123', umpire_id: 'umpire-456' },
      error: null,
    });
    ...
    expect(data.message).toBe('審判を変更しました');
    expect(data.data.umpire_id).toBe('umpire-456');
    expect(mockUpdate).toHaveBeenCalledWith({ umpire_id: 'umpire-456' });
  });
  ...
  it('should return 500 on server error', async () => {
    ...
    expect(data.error).toContain('審判の変更に失敗しました');
