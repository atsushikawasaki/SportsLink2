    ...
    const fetchTournaments = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const entered = allTournaments.filter((t: Tournament) => 
                    t.status === 'published' || t.status === 'finished'
                ).length;
                const publicCount = allTournaments.filter((t: Tournament) => 
            ...
            // 審判割り当て試合取得
            if (user?.id) {
                const matchesRes = await fetch(`/api/matches/umpire/${user.id}`);
